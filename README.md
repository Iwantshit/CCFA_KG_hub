...
以目标为导向，我提出了不同于图分类的一种新分类想法——节点分类。不同于每个论文都提取一次特征、画出知识图谱（可能old work也只做到了知识图谱的制作）在进行图分类任务，我的方法更加快捷高效，
flops仅有感人的554.25KMac，更值得说明的是，这个554.25KMac还是在微调bert五层的情况下出现的，仅用bert可能甚至更少。

除此外，对于文件夹中的文件，由于存储空间原因，我删去了诸如论文pdf文件等一些原始文件，保留了.txt等过程文件。

bert模型调用需要openai库、可能需要梯子环境等，我写的代码应该是比之前的代码更容易看懂的，我排的还是比较规范的，而且代码量其实不多，理解了就能用。
